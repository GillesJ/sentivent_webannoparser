...
import operator
import pandas as pd
from parser import WebannoProject, Element
from iteration_utilities import duplicates

pd.set_option("display.max_columns", None)


# TODO fix token_span getter
# TODO fix token_id usage: needed for creating unique elements in position of document

# represent corpus as list of token ids:


class AgreementStudy:
    def __init__(self, annotation_documents=None):

        self.annotation_documents = (
            dict() if annotation_documents is None else annotation_documents
        )
    ...
    def from_project(cls, project):
        """
        Get the annotation_documents dict from the project.
        Group the docs in the project object by annotator.
        :return:
        """

        def check_equal(lst):  # for test: assert same len and assert same titles
            return not lst or lst.count(lst[0]) == len(lst)

        annotation_documents = {}
        key = lambda x: x.annotator_id
        project.annotation_documents.sort(key=key)
        ...
        if not check_equal(
            [[d.title for d in x] for x in annotation_documents.values()]
        ):
            raise ValueError("All titles of the documents are the same in IAA study.")

        return cls(annotation_documents)

    @classmethod
    def from_documents(cls, ref, gold):
        """
        ...
        :return:
        """
        ref_docs = [ref] if not isinstance(ref, list) else ref
        gold_docs = [gold] if not isinstance(gold, list) else ref

        if len(ref_docs) != len(gold_docs):
            raise (
                ValueError(
                    "The amount of gold documents and reference documents needs to match."
                )
            )

        for ref, gold in zip(ref_docs, gold_docs):
            ref.annotator_id_orig = ref.annotator_id
            ref.annotator_id = "ref"
            gold.annotator_id_orig = gold.annotator_id
            gold.annotator_id = "gold"

        annotation_documents = {"ref": ref_docs, "gold": gold_docs}
    ...
    def _match_partial_span(self, gold_anns, system_anns, system_annotator_id):
        ...
        gold_tokens = [ann.get_extent_tokens() for ann in gold_anns]  # testing
        system_tokens = [ann.get_extent_tokens() for ann in system_anns]
        token_combination = list(itertools.product(gold_tokens, system_tokens))
        token_id_combination = (
            ([t.get_token_id() for t in gold_tok], [t.get_token_id() for t in sys_tok])
            for gold_tok, sys_tok in token_combination
        )

        scores = np.fromiter(
            (self.dice_coefficient(a, b) for (a, b) in token_id_combination), float
        )
        ...
        annotation_combination = np.array(
            list(itertools.product(gold_anns, system_anns))
        )
    ...
    def _match_by_type(self, gold_anns, system_anns, system_annotator_id):
        def set_match(match_in_window, gold_ann, window=None):
            ...
            gold_ann_begin_token_pos = min(
                t.index for t in gold_ann.get_extent_tokens()
            )
            gold_ann_end_token_pos = max(t.index for t in gold_ann.get_extent_tokens())
            gold_ann_token_text = [t.text for t in gold_ann.get_extent_tokens()]

            for m in match_in_window:
                match_ann_begin_token_pos = min(t.index for t in m.get_extent_tokens())
                match_ann_end_token_pos = max(t.index for t in m.get_extent_tokens())
                match_ann_token_text = [t.text for t in m.get_extent_tokens()]

                token_distance = max(
                    gold_ann_begin_token_pos - match_ann_end_token_pos,
                    match_ann_begin_token_pos - gold_ann_end_token_pos,
                )
                dice_sim_score = self.dice_coefficient(
                    gold_ann_token_text, match_ann_token_text
                )
                match = {
        ...
        for gold_ann in gold_anns:
            ...
            selection_val = (
                gold_ann.event_type
            )  # TODO make the settings_attrib a keyword argument
            type_match = list(
                filter(lambda x: x.event_type == selection_val, system_anns)
            )
            ...
            match_in_sentence = list(
                filter(
                    lambda x: [s.element_id for s in x.in_sentence]
                    == gold_in_sentence_id,
                    type_match,
                )
            )
    ...
    def match_candidate_annotations(self, annotation_name, replace_id=False):
        """
        ...
        for annotated_docs in zip(
            *self.annotation_documents.values()
        ):  # iterate over 3 docs
            ...
            for (gold_doc, system_doc) in itertools.permutations(annotated_docs, 2):
                ...
                # set needed attributes on gold_anns: attrib for containing the system matches, change
                for gold_ann in gold_anns:
                    if replace_id:
                        gold_ann.annotator_id = "gold"
                    gold_ann.matched_to_gold = {}
                ...
                # set needed attributes on sys_anns
                for sys_ann in system_anns:
                    if replace_id:
                        sys_ann.annotator_id = "ref"
                    sys_ann.matched_to_gold = {}
                # match partial span matches
                self._match_partial_span(
                    gold_anns, system_anns, system_doc.annotator_id
                )
    ...
    #
    #     if not hasattr(next(iter(self.annotation_documents.values()))[0], annotation_attrib):
    ...
    #         if not hasattr(next(iter(self.annotation_documents.values()))[0], self.discont_map[annotation_attrib]):
    #             raise ValueError(f"Attribute {self.discont_map[annotation_attrib]} is not on document.")
    #
    #     for doc in next(iter(self.annotation_documents.values())):
        ...
        for anno_id, gold_docs in self.annotation_documents.items():
            ...
            for doc in gold_docs:
                ...
                if attribute is not None:
                    annotation = list(
                        filter(
                            lambda x: getattr(x, attribute[0]) == attribute[1],
                            annotation,
                        )
                    )
                    pass
    ...
    def is_attribute_accurate(self, gold, match, attribute_name):
        ...
        if attribute_name == "event_subtype":
            ...
            match_val = getattr(match, "event_type") + str(
                getattr(match, "event_subtype")
            )
        else:
    ...
    def score_ere_nugget(
        self,
        all_gold_annotations,
        attributes=["event_type", "event_subtype", "polarity_negation", "modality"],
    ):

        if not self.ere_matched:
            raise ValueError("Types have not yet been matched.")
        ...
        for gold_anno_id, gold_annotations in all_gold_annotations.items():
            ...
            sys_ann_ids = [
                k for k in self.annotation_documents.keys() if k != gold_anno_id
            ]
            metrics_template = {
                "tp": 0.0,
                "tp_attrib": 0.0,
            ...
            for gold_ann in gold_annotations:
                ...
                for sys_anno_id, sys_matches in gold_ann.selected_match.items():
                    if sys_matches is not None:
                        ...
                        print(scores, sys_anno_id, gold_ann.annotator_id)
                        scores[sys_anno_id][
                            "n_matches"
                        ] += 1.0  # count the valid span matches to not penalize twice for span errors
                        for sys_match in sys_matches:
                            sys_match[
                                "attribute_match"
                            ] = (
                                {}
                            )  # collect if the attribute matches True False: needed for computing tp_attrib
                            for attribute_name in attributes:
                                ...
                                sys_match["attribute_match"][
                                    attribute_name
                                ] = attribute_is_accurate
                                if attribute_is_accurate:
                                    scores[sys_anno_id][attribute_name] += 1.0 / len(
                                        sys_matches
                                    )  # attribute is accurate count
                        ...
                        max_span_dice_match = max(
                            sys_matches, key=lambda x: x["position_dice_sim_score"]
                        )
                        scores[sys_anno_id]["tp"] += max_span_dice_match[
                            "position_dice_sim_score"
                        ]
                        scores[sys_anno_id]["tp_relaxed"] += 1.0
                        if all(
                            max_span_dice_match["attribute_match"].values()
                        ):  # all attributes are accurate is the condition for attribute based TP
                            scores[sys_anno_id]["tp_attrib"] += max_span_dice_match[
                                "position_dice_sim_score"
                            ]
            ...
            for sys_anno_id, metrics in scores.items():
                ...
                metrics["p_attrib"] = metrics["tp_attrib"] / (
                    metrics["tp_attrib"] + metrics["fp_attrib"]
                )
                ...
                metrics["f1_span"] = (2 * metrics["p"] * metrics["r"]) / (
                    metrics["p"] + metrics["r"]
                )
                metrics["f1_span_alt"] = (2 * metrics["p_alt"] * metrics["r"]) / (
                    metrics["p_alt"] + metrics["r"]
                )
                metrics["f1_attrib"] = (
                    2 * metrics["p_attrib"] * metrics["r_attrib"]
                ) / (metrics["p_attrib"] + metrics["r_attrib"])
                metrics["f1_attrib_alt"] = (
                    2 * metrics["p_attrib_alt"] * metrics["r_attrib"]
                ) / (metrics["p_attrib_alt"] + metrics["r_attrib"])
                metrics["f1_span_relaxed"] = (
                    2 * metrics["p_relaxed"] * metrics["r_relaxed"]
                ) / (metrics["p_relaxed"] + metrics["r_relaxed"])
                metrics["f1_attrib"] = (
                    2 * metrics["p_attrib"] * metrics["r_attrib"]
                ) / (metrics["p_attrib"] + metrics["r_attrib"])
                metrics["f1_attrib_relaxed"] = (
                    2 * metrics["p_attrib_relaxed"] * metrics["r_attrib_relaxed"]
                ) / (metrics["p_attrib_relaxed"] + metrics["r_attrib_relaxed"])
                # compute acc
                for attribute_name in attributes:
                    metrics[f"{attribute_name}_acc"] = (
                        metrics[attribute_name] / metrics["n_matches"]
                    )
    ...
    def average_ere_scores(self, scores_all):
        ...
        for k_gold, system_scores in scores_all.items():
            ...
            all_results["avg_annotator_scores"][k_gold] = {
                k: sum(t[k] for t in sys_scores) / len(sys_scores)
                for k in sys_scores[0]
            }

        avg_scores_by_anno = list(all_results["avg_annotator_scores"].values())
        all_results["avg_all_scores"] = {
            k: sum(t[k] for t in avg_scores_by_anno) / len(avg_scores_by_anno)
            for k in avg_scores_by_anno[0]
        }
    ...
    def select_ere_matches(self, annotation_name):
        ...
        for gold_anno_id, gold_annotations in all_gold_annotations.items():
            ...
            for gold_ann in gold_annotations:
                ...
                for (
                ...
                ) in gold_ann.match_candidates.items():
                    ...
                    if candidate_system_matches is not None:
                        candidate_system_matches = list(
                            filter(
                                lambda x: "partial_token_sim" in x,
                                candidate_system_matches,
                            )
                        )
                        ...
                        if candidate_system_matches is not None:
                            if len(candidate_system_matches) > 1:
                                ...
                                max_sim = max(
                                    candidate_system_matches,
                                    key=lambda x: x["position_dice_sim_score"],
                                )["position_dice_sim_score"]
                                selected_match = [
                                    x
                                    for x in candidate_system_matches
                                    if x["position_dice_sim_score"] == max_sim
                                ]
                            else:
                                selected_match = candidate_system_matches

                            selected_match = self.resolve_match_coordination(
                                gold_ann, selected_match
                            )
                            ...
                            for sel_match in selected_match:
                                ...
                                if ann_match not in already_matched[system_anno_id]:
                                    if (
                                        gold_ann.selected_match[system_anno_id]
                                        is not None
                                    ):
                                        gold_ann.selected_match[system_anno_id].append(
                                            sel_match
                                        )
                                    else:
                                        gold_ann.selected_match[system_anno_id] = [
                                    ...
                                    ann_match.matched_to_gold.setdefault(
                                        gold_anno_id, []
                                    ).append(gold_ann)
        ...
        duplicates_found, _ = self.check_duplicate_selected_match(
            "events"
        ...
        for gold_anno_id, gold_annotations in all_gold_annotations.items():
            for gold_ann in gold_annotations:
                for other_anno_id, match in gold_ann.match_candidates.items():
                    print(
                        f"\n{'-'*160}"
                        f"\nMATCH FROM {gold_anno_id} TO {other_anno_id} IN {gold_ann.document_title}"
                    ...
                    if gold_ann.match_candidates[other_anno_id] is not None:
                        if gold_ann.selected_match[other_anno_id] is None:
                            ...
                            for match in gold_ann.match_candidates[other_anno_id]:
                                matched_to = self._get_annotation_from_match(
                                    match
                                ).matched_to_gold
    ...
    def check_duplicate_selected_match(self, annotation_name):
            ...
            for gold_ann in gold_annotations:
                for sys_anno_id, sys_match in gold_ann.selected_match.items():
                    if sys_match is not None:
                        sys_match = [
                            self._get_annotation_from_match(m) for m in sys_match
                        ]
                        flat_matches[gold_anno_id].setdefault(sys_anno_id, []).extend(
                            sys_match
                        )
        ...
        for gold_anno_id, sys_matches in flat_matches.items():
            ...
            for sys_anno_id, matches in sys_matches.items():
                ...
                duplicate_entries[gold_anno_id][sys_anno_id] = [
                    match for match in matches if match in dupes
                ]
    ...
    def get_selected_match_counts(self, annotation_name):
        ...
        selected_matches = {
            k: {kk: [] for kk in [kkk for kkk in all_anns.keys() if kkk is not k]}
            for k in all_anns.keys()
        }
        for anno_id, annotations in all_anns.items():
            for ann in annotations:
                for other_anno_id, matches in ann.selected_match.items():
                    if matches:
                        selected_matches[anno_id].setdefault(other_anno_id, []).append(
                            matches
                        )
        cnt = {
            g: {
                s: (len(match), round(100 * len(match) / len(all_anns[g]), 2))
                for s, match in m.items()
            }
            for g, m in selected_matches.items()
        }
        return cnt

    def _select_partial_span_match(matches):
        # todo add double spn and maching
        selected_match = None
        ...
        if partial_span_matches:
            if len(partial_span_matches) > 1:
                selected_match = max(
                    partial_span_matches, key=lambda x: x["position_dice_sim_score"]
                )
            else:
    ...
    def _select_match_by_key(matches, match_key=None):
        ...
        if match_key not in valid_match_keys:
            ...
            window_matches = list(
                filter(
                    lambda x: "token_distance" in x and x["token_distance"] <= window,
                    matches,
                )
            )
        ...
        if window_matches:
            ...
            if len(window_matches) > 1:
                selected_match = max(
                    window_matches,
                    key=lambda x: (x["dice_sim_score"], -x["token_distance"]),
                )
            else:
    ...
    def select_custom_match(
        self, annotation_name, strategy=["partial_token_sim", "in_window_sentence"]
    ):
        all_gold_annotations = self.collect_annotations_by_annotator(annotation_name)
        for gold_anno_id, gold_annotations in all_gold_annotations.items():
            already_matched = {}  # set for checking unique annotations
    ...
    def plot_confusion_matrix(
        self, annotation_pairs, attrib="event_type", filepath="confusion_matrix.png"
    ):
        ...
        for gold_anno_id, gold_annotations in annotation_pairs.items():
            gold_attrib_vals = []
            match_attrib_vals = {
                k: [] for k in annotation_pairs.keys() if k != gold_anno_id
            }
            for gold_ann in gold_annotations:
                # for every match:
                ...
                for sys_anno_id, sys_matches in gold_ann.selected_match.items():
                    if sys_matches is not None:
                        match_attrib = getattr(
                            self._get_annotation_from_match(sys_matches[0]), attrib
                        )
                    else:
                        match_attrib = "None"
        ...
        class_names.append(
            class_names.pop(class_names.index("None"))
        )  # move None to last position for plot aesthetics
...
def score_document_ere(ref, gold):
    """
    Provides ERE-like nugget scoring of a reference document against a gold standard document.
    TODO add Bibtex reference to paper describing ERE scoring method
    :param ref: Reference document that is compared against gold standard.
    :param gold: Gold standard document against which the reference is compared.
    :return: ERE nugget scores dict.
    """
    agr = AgreementStudy().from_documents(ref, gold)
    agr.match_candidate_annotations("events", replace_id=True)
    agr.select_ere_matches("events")  # select the partial span ere nugget match
    all_attributes = ["event_type", "event_subtype", "polarity_negation", "modality"]

    # score ere nuggets
    all_gold_annotations = agr.collect_annotations_by_annotator("events")
    ere_nugget_results = agr.score_ere_nugget(
        all_gold_annotations, attributes=all_attributes
    )

    return ere_nugget_results
...
if __name__ == "__main__":
    ...
    if not Path(opt_fp).is_file() or FROM_SCRATCH:
        ...
        event_project.annotation_document_fps = list(
            filter(iaa_filter_func, event_project.annotation_document_fps)
        )
    ...
    iaa_study = AgreementStudy().from_project(event_project)
    iaa_study.match_candidate_annotations(
        "events"
    )  # create all possible matches according to different criteria
    ...
    all_attributes = [
        ...
        "polarity_negation",
        "modality",
    ]  # TODO think about event participants

    # score ere nuggets
    all_gold_annotations = iaa_study.collect_annotations_by_annotator("events")
    ere_nugget_results = iaa_study.score_ere_nugget(
        all_gold_annotations, attributes=all_attributes
    )
    ...
    separate_attributes_results = {
        attrib: iaa_study.score_ere_nugget(all_gold_annotations, attributes=[attrib])
    ...
    ia_type_f1 = separate_attributes_results["event_type"]["avg_all_scores"][
        "f1_attrib_alt"
    ]
    ia_subtype_f1 = separate_attributes_results["event_subtype"]["avg_all_scores"][
        "f1_attrib_alt"
    ]
    ia_polarity_f1 = separate_attributes_results["polarity_negation"]["avg_all_scores"][
        "f1_attrib_alt"
    ]
    ia_modality_f1 = separate_attributes_results["modality"]["avg_all_scores"][
        "f1_attrib_alt"
    ]
    ...
    for event_type in set(all_event_types):
        type_gold_annotations = iaa_study.collect_annotations_by_annotator(
            "events", attribute=("event_type", event_type)
        )
        type_ere_nugget_results = iaa_study.score_ere_nugget(
            type_gold_annotations, attributes=all_attributes
        )
    ...
    df = (df[["f1_span_alt", "f1_attrib_alt"]] * 100).round().astype(int)
    fig = plt.figure(figsize=[9, 5])
    figax = fig.add_subplot(111)
    ax = df.plot(ax=figax, kind="bar")
    (n_cat, n_bars) = df.shape
    adjacent_patches = [
        ax.patches[i + (j * n_cat)] for i in range(n_cat) for j in range(n_bars)
    ]
    for adj_patches in util.n_iterate(adjacent_patches, n_bars):
        p_prev_height = None
        for p in adj_patches:
            if p_prev_height != p.get_height():
                ax.annotate(
                    p.get_height(),
