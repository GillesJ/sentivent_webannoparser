...
def annotations_to_df(project):
    ...
    all_df = pd.DataFrame(to_data(u) for uname in unames for u in project.get_annotation_from_documents(uname))
    ...
    all_df["polarity_negation"] = all_df["polarity_negation"].replace({"false": "positive", "true": "negative"})
    all_df["modality"] = all_df["modality"].replace({"false": "certain", "true": "other"})
...
def plot_sentiment_polarity_chart(all_df, opt_fp):
    ...
    df = all_df.groupby("annotation_unit").polarity_sentiment.value_counts(normalize=True).unstack() * 100
    df_count = all_df.groupby("annotation_unit").polarity_sentiment.value_counts(normalize=False).unstack()
    df = df[cats_in_order]
    ...
    # set counts on bar
    for c, col_patches in enumerate(chunker(ax.patches, df.shape[0])): # patches iterate over columns
