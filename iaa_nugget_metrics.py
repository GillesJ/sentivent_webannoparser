...
class AgreementStudy:
    ...
    def __init__(self, annotation_documents=None):
        ...
        self.annotation_documents = dict() if annotation_documents is None else annotation_documents
    ...
    def from_project(cls, project):
        ...
        if not check_equal([[d.title for d in x] for x in annotation_documents.values()]):
    ...
    def from_documents(cls, ref, gold):
        ...
        if len(ref_docs) != len(gold_docs):
            raise(ValueError("The amount of gold documents and reference documents needs to match."))
    ...
    def _match_partial_span(self, gold_anns, system_anns, system_annotator_id):
        ...
        token_id_combination = (([t.get_token_id() for t in gold_tok], [t.get_token_id() for t in sys_tok]) for gold_tok, sys_tok in token_combination)
    ...
    def _match_by_type(self, gold_anns, system_anns, system_annotator_id):
        ...
        def set_match(match_in_window, gold_ann, window=None):
            ...
            gold_ann_begin_token_pos = min(t.index for t in gold_ann.get_extent_tokens())
...
def score_document_ere(ref, gold):
    ...
    all_attributes = ["event_type", "event_subtype", "polarity", "modality"]

    # score ere nuggets
    all_gold_annotations = agr.collect_annotations_by_annotator("events")
    ere_nugget_results = agr.score_ere_nugget(all_gold_annotations, attributes=all_attributes)
...
if __name__ == "__main__":
    ...
    adjacent_patches = [ax.patches[i + (j * n_cat)] for i in range(n_cat) for j in range(n_bars)]
